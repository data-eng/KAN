...
\begin{frame}{Architecture: Comparison with MLP}
    ...
    \begin{table}[h!]
    	...
    	\begin{tabular}{cc}
    		\toprule
    		\textbf{In KAN...} & \textbf{In MLP...} \\
    		\midrule
    		\midrule
    		Relaxing KART assumptions makes  & UAT guarantees\\
    		 KAN a function approximator & 	approximation\\
    	 	\midrule
    		KART provides no method to compute   & UAT lacks guarantees   \\
    		the relative functions & on shallow MLP size \\
    		\bottomrule
    	\end{tabular}
    \end{table}
\end{frame}
...
\begin{frame}{Architecture: Comparison with MLP}
    ...
    \begin{columns}
    	\begin{column}{6.6cm}
    		\begin{center}
    			\includegraphics[width=6.5cm]{contents/images/kans_vs_mlps}
    			\end{center}
    	\end{column}
    	...
    	\begin{itemize}
    		...
    		outputs as a matrix of 1D functions, stacking these layers \textit{(...note how we are drifting further away from KART)}\vspace{0.3em}
    	\end{itemize}
...
\begin{columns}
	\begin{column}{6cm}
		...
		\item Let us present the B-splines in depth \textit{starting from the basics..} \vspace{0.3em}
	\end{itemize}
	\end{column}
	
	\begin{column}{5cm}
		\begin{center}
		\includegraphics[width=5cm]{contents/images/spline_example.png}
	\end{center}
...
\begin{frame}{B-splines}
    \begin{itemize}
        ...
        \item Additionally, there exists a unique combination of B-splines for any given spline \textcolor{citations}{(de Boor, 1978)} \vspace{0.5em}
...
\begin{frame}{B-splines: Spline interpolation}
    \begin{itemize}
        ...
        \item This process arises naturally from the formal definition of B-splines: it recursively defines each basis from a grid \textcolor{citations}{(Kaihuai, 1998)},
...
\begin{frame}{B-splines and KANs}
    \begin{itemize}
        \item In KANs, each activation function is a $k^{\text{th}}$ order B-spline, with a $G$-valued and uniformly spaced grid  \vspace{0.5em}
        \item For each activation function what is left to learn is the coefficients of the spline \vspace{0.5em}
        \item Thus, for a KAN of width $W$ and depth $D$ the $\#$training parameters is $\mathbb{O}(W^2DG)$ \vspace{0.5em}
        \item The respective number of parameters for an MLP is $\mathbb{O}(W^2D)$, but KAN promises convergence with smaller widths \vspace{0.5em}
    \end{itemize}
\end{frame}
