...
\begin{frame}{Beating the COD (?)}
   \begin{itemize}
       ...
       \item \textit{However}, we should note that the current (spline) implementation of KANs is not as optimized as the one for MLPs, so in practice the training times for high-dimensional data are comparable
   \end{itemize}
\end{frame}
