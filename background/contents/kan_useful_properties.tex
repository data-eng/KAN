...
\begin{frame}{Interpretability: Motivation}
    \begin{itemize}
        ...
        \item But, this is only useful when we a priori know the structure of the KAN and know what we expect to see (i.e. we are somewhat \textit{domain experts})\vspace{0.5em}
...
\begin{frame}{Kind reminder: KAN is yet another function approximator!}
	\vspace{-0.8cm}
	\begin{figure}
		\noindent
		\begin{center}
...
\begin{frame}{Beating the COD (?)}
   \begin{itemize}
       \item The \textbf{curse of dimensionality (COD)} refers to the exponential growth in volume associated with adding more dimensions to a space 
       \vspace{0.3em}
       \item UAT: a neural network with one hidden layer can approximate any function, but as the dimensionality of the input increases, the number of neurons needed grows exponentially \vspace{0.3em}
       ...
       \item So, KAN's size will not increase exponentially as the data dimensions increase, but its grid should become more fine-grained (... \textit{it switches the complexity}) \vspace{0.3em}
       \item \textit{However}, we note that the current (spline) implementation of KANs is not as optimized as the one for MLPs, so in practice the training times for high-dimensional data are comparable
   \end{itemize}
\end{frame}
...
\begin{frame}{Continual Learning}
   \begin{itemize}
   		\item \textbf{To vibe with this slide, wear the hat of the domain expert!} \vspace{0.3em}
