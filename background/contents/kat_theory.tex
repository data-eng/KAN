...
\item Arnold (1957): Only two-variable functions are really needed
...
\begin{columns}
  ...
  \begin{column}{7cm}
  Construct a similar series for the second dimension; Add.
  \vskip 1em
...
Step 1 constructed the non-overlapping intervals
\vspace{0.5em}
\begin{itemize}
\item Step 1 also includes the proof that it is always possible to
  construct these intervals so that every point in $E^2$ is covered
  by at least one non-gap area of one of these systems of `platforms'
\item Intuition: Do not align the gaps  
\end{itemize}
...
\item But: There is no value that is over a gap in \emph{all} systems of
  platforms
\end{itemize}
